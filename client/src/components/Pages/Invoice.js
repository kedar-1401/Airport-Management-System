...
const Invoice = () => {
  ...
  const loadData = async () => {
    const Returnresponse = await Axios.get(
      "http://localhost:5000/api/auth/invoicefares"
    );
  ...
  useEffect(() => {
    ...
    console.log(initialState.cl_id + ' '+ initialState.sc_id);
    Axios.post("http://localhost:5000/api/auth/UpdateFlightBooking",{
        id:initialState.sc_id,
    })
    Axios.get(`http://localhost:5000/api/auth/invoice/${initialState.cl_id}`).then((resp) =>
      setData({ ...resp.data[0] })
    );
    loadData();
  }, []);
  const foo = async () => {
    await Axios.post("http://localhost:5000/api/auth/invoiceconfirm", {
      id: initialState.sc_id,
      departure: user.departure,
    });
    await Axios.post("http://localhost:5000/api/auth/invoiceconfirmAgain", {
      id: initialState.cl_id,
      flight_no: user.flight_no,
      fares: user.price.substr(2, 6),
    });
    Axios.delete('http://localhost:5000/api/auth/removesearch');
