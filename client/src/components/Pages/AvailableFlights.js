...
const AvailableFlights = () => {
  ...
  const loadData = async () => {
    const response = await Axios.get("http://localhost:5000/api/auth/SearchFlights");
    const lastIndex = response.data.length - 1;
    console.log('Available FLights: ' + response.data[0]);
    initialState.fb_id = response.data[lastIndex].fb_id;
    console.log(initialState.fb_id);
    initialState.departure = response.data[lastIndex].departure;
    initialState.arrival = response.data[lastIndex].arrival;
    initialState.departureDate = response.data[lastIndex].departureDate;
    initialState.returnDate = response.data[lastIndex].returnDate;
    initialState.class = response.data[lastIndex].class;
    initialState.price = response.data[lastIndex].price;
    console.log('fb_id: ' + initialState.fb_id)
    console.log('departureDate: ' + initialState.departureDate)
    console.log('price: ' + initialState.price)

    console.log('return: ' + initialState.returnDate)
    const Returnresponse=await Axios.post("http://localhost:5000/api/auth/AvailableFlights",{
      departureDate:initialState.departureDate,
      returnDate:initialState.returnDate,
      fares:initialState.price,
    });
    console.log('data from available flights'+Returnresponse.data)
